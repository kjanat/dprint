...
# from a pipe (ex. a shell process substitution, or a fifo from mkfifo)
dprint fmt --config <(sed 's/dist/build/' dprint.json)
# from stdin, which is what `-`, `/dev/stdin` and a --config with no value at all mean
...
A value counts as the configuration itself when it starts with `{` and ends with `}`, so a configuration file named `{project}.json` is still read as a path. A file whose name both starts and ends with a brace is ambiguous—name it as `./{project}` to be certain.
...
Remote configuration files (`extends` or `--config` with a URL) are checked for changes the next time they are resolved after their freshness period, which is an hour or the `max-age` of a response marked `immutable` when that is longer (see [caching of remote configuration](/config#caching-of-remote-configuration)), so clearing the cache is only necessary to pick up a change immediately.
